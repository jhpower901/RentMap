...
값이 비어있으면 worker는 silent no-op — 이벤트는 큐에 쌓이지만 발송하지 않습니다 (나중에 URL 설정하면 자동 drain).
...
scheduler가 매 1분 워커를 호출하므로 다음 분에 자동 발송 시작.
...
`deliverable`이 발송 대기. 활성화하면 분당 25건씩 drain.
...
- Discord 한도: 분당 30건 / webhook. Worker는 분당 25건 cap (헤드룸).
...
worker는 URL 없으면 silent no-op. 큐는 그대로 차오름 — URL 다시 설정하면 자동 drain.
